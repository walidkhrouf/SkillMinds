  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/tutorials/generate-image",
        { content: formData.content }
      );
      setImageUrl(response.data.imageUrl);
    } catch (error) {
      console.error("Error generating image:", error);
      setSubmitError("Failed to generate image");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <section className="tutorial-section">
        <form onSubmit={handleSubmit} className="tutorial-form enhanced">
          ...
          {submitError && <p className="tutorial-detail__error">{submitError}</p>}
          ...
          <div className="tutorial-form__field flex-container">
